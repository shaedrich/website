...
<details><summary>Framework maintainers</summary>
...
I feel like this post has a lot of good nuggets but doesn't make the point Jatin and I tried to make (although Jatin's part was extremely abstract): that this design through some miracle manages to leave data to one side and provide a tiny hook for frameworks to watch reactive things and flush them
...
effects can't be implemented as a core feature (for all frameworks to use) because frameworks have different semantics for when the effect actually runs.
...
- must subscribe to "derived" purism (no effects), or implement a `using`/Resource-based effect as a util in your library, but I wouldn't recommend this.
...
[signal-utils]: github.com/NullVoxPopuli/signal-utils/
