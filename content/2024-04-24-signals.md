...
Additionally, the `Watcher` implementation requires "unsubscribing", like you would with a `ResizeObserver` or `MutationObserver` or `eventListener`. And in order to safely unsubscribe, you either need an ownership implementation (e.g.: when a parent "container" is destroyed, also destroyed / clean up / unsubscribe from all child things). 

_But_, after some discussions with early reviewers of the proposal, some folks are perfectly fine with managing cleanup themselves -- it's something that developers would be used to if they aren't using a framework which reduces the need to think about cleanup altogether. Afterall, UI frameworks are not the only audience for the Signals proposal. 

Also **there are many kinds of effects**. The different kinds of effects depend on timing, semantics, and batching.
...
In contrast with Scheduled or Lifecycle Effects, this behavior would be observed, and values are not immediately available:
```js
const a = new Signal.State(0);
const b = new Signal.State(0);
...
For example, back in my React days, I've seen a lot of running async functions using effects -- but it's not needed. We can use an "impure" computed, which invokes an async function gives us a reactive state back to inspect the lifecycle states of the returned Promise. In [signal-utils' `signalFunction`](https://github.com/proposal-signals/signal-utils/blob/main/src/async-function.ts),
```ts
...
The dream is that once frameworks integration with native signals, whole ecosystems begin to start being able to share resources with one another. 
...
Higher level abstractions should also be possible, and some of these are explored in [signal-utils][signal-utils], such as in ecosysetms that encourage classes, they may implement a decorator which allows "just regular property access" to continue working:
```jsx
